...
class FontFamily:
	...
	def _buildboundingboxtable(self):
		if self.boundingBoxes and self._rawoffsettable[:6] == '\0\0\0\0\0\6':
			boxes = list(self.boundingBoxes.items())
			boxes.sort()
	...
	def _buildglyphwidthtable(self):
		...
		tables = list(self.widthTables.items())
		tables.sort()
	...
	def _buildkerningtables(self):
		...
		data = [struct.pack('>h', numberofentries - 1)]
		tables = list(self.kernTables.items())
		tables.sort()
		for stylecode, table in tables:
	...
	def _unpackstylestrings(self):
		...
		for i in list(self.widthTables.keys()):
			index = self.styleIndices[i]
			if index == 1:
				psNames[i] = self.styleStrings[0]
			else:
				style = self.styleStrings[0]
				codes = list(map(ord, self.styleStrings[index - 1]))
				for code in codes:
	...
	def _packstylestrings(self):
		nameparts = {}
		splitnames = {}
		for style, name in list(self.psNames.items()):
			split = splitname(name, self.ffFamilyName)
			splitnames[style] = split
			for part in split:
				nameparts[part] = None
		del nameparts[self.ffFamilyName]
		nameparts = list(nameparts.keys())
		nameparts.sort()
		items = list(splitnames.items())
		items.sort()
		numindices = 0
	...
	def _buildglyphencodingsubtable(self):
		...
		items = list(self.glyphEncoding.items())
		items.sort()
	...
	def parse(self):
		...
		for fond in self.fonds:
			...
			if hasattr(fond, "psNames") and fond.psNames:
				psNames = list(fond.psNames.values())
				psNames.sort()
