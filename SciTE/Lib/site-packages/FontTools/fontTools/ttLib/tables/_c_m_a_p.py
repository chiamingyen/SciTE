...
class CmapSubtable:
	...
	def _writeCodes(self, codes, writer):
		if (self.platformID, self.platEncID) == (3, 1) or (self.platformID, self.platEncID) == (3, 10) or self.platformID == 0:
			from fontTools.str import Unicode
			isUnicode = 1
		else:
			isUnicode = 0
...
class cmap_format_0(CmapSubtable):
	...
	def decompile(self, data, ttFont):
		...
		names = list(map(self.ttFont.getGlyphName, glyphIdArray))
...
class cmap_format_2(CmapSubtable):
	...
	def fromXML(self, xxx_todo_changeme2, ttFont):
...
class cmap_format_6(CmapSubtable):
	...
	def compile(self, ttFont):
		...
		if codes: # yes, there are empty cmap tables.
			codes.sort()
			lenCodes = len(codes)
			assert codes == list(range(codes[0], codes[0] + lenCodes))
			firstCode = codes[0]
			valueList = list(map(operator.getitem, [cmap]*lenCodes, codes))
			valueList = list(map(ttFont.getGlyphID, valueList))
	...
	def fromXML(self, xxx_todo_changeme4, ttFont):
...
class cmap_format_12(CmapSubtable):
	...
	def decompile(self, data, ttFont):
		...
		for i in range(self.nGroups):
			...
			gids += list(range(glyphID, glyphID + lenGroup))
		self.data = data = None
		self.cmap = cmap = {}
		lenCmap = len(gids)
		glyphOrder = self.ttFont.getGlyphOrder()
	...
	def fromXML(self, xxx_todo_changeme5, ttFont):
...
class cmap_format_14(CmapSubtable):
		...
		for n in range(self.numVarSelectorRecords):
			...
			if defOVSOffset:
				...
				for r in range(numValues):
					...
					glyphList = [None]*cnt
					localUVList = list(zip(baseUVList, glyphList))
					try:
	...
	def fromXML(self, xxx_todo_changeme6, ttFont):
