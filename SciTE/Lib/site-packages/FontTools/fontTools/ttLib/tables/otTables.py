...
class Coverage(FormatSwitchingBaseTable):
	...
	def fromXML(self, xxx_todo_changeme, font):
...
class LookupList(BaseTable):
	def preCompile(self):
		...
		liRange = list(range(lookupCount))
		extTables = []
		for li in liRange:
			...
			siRange = list(range(subtableCount))
			for si in siRange:
...
class SingleSubst(FormatSwitchingBaseTable):
	...
	def postRead(self, rawTable, font):
		...
		if self.Format == 1:
			...
			inputGIDS = list(map(doModulo, inputGIDS)) 
			outGIDS = [ glyphID + delta for glyphID in inputGIDS ]
			outGIDS = list(map(doModulo, outGIDS)) 
	...
	def fromXML(self, xxx_todo_changeme1, font):
...
class ClassDef(FormatSwitchingBaseTable):
	...
	def toXML2(self, xmlWriter, font):
		items = list(self.classDefs.items())
		items.sort()
		for glyphName, cls in items:
			xmlWriter.simpletag("ClassDef", [("glyph", glyphName), ("class", cls)])
			xmlWriter.newline()
	
	def fromXML(self, xxx_todo_changeme2, font):
...
class AlternateSubst(FormatSwitchingBaseTable):
	...
	def toXML2(self, xmlWriter, font):
		items = list(self.alternates.items())
		items.sort()
	...
	def fromXML(self, xxx_todo_changeme3, font):
...
class LigatureSubst(FormatSwitchingBaseTable):
	...
	def toXML2(self, xmlWriter, font):
		items = list(self.ligatures.items())
		items.sort()
	...
	def fromXML(self, xxx_todo_changeme4, font):
...
def splitAlternateSubst(oldSubTable, newSubTable, overflowRecord):
	...
	oldAlts = list(oldSubTable.alternates.items())
	oldAlts.sort()
	oldLen = len(oldAlts)
...
def splitLigatureSubst(oldSubTable, newSubTable, overflowRecord):
	ok = 1
	newSubTable.Format = oldSubTable.Format
	oldLigs = list(oldSubTable.ligatures.items())
	oldLigs.sort()
	oldLen = len(oldLigs)
...
def _buildClasses():
	...
	for base, alts in list(_equivalents.items()):
		base = namespace[base]
		for alt in alts:
	...
	for lookupEnum in list(lookupTypes.values()):
		for enum, cls in list(lookupEnum.items()):
			cls.LookupType = enum
