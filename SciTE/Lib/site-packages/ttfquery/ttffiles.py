...
FILENAME, MODIFIERS, SPECIFICNAME, FONTNAME, FAMILY = list(range(5))
...
class Registry(object):
	...
	def familyMembers( self, major, minor=None ):
		...
		if not minor:
			result = []
			for key,set in list(self.families.get(major,{}).items()):
				result.extend( list(set.keys()))
			return result
		minor = minor.upper()
	...
	def matchName( self, name, single=0 ):
		...
		if name.upper() in self.families:
			for general in self.familyMembers( name ):
				...
				result[general] = 1
		testname = name.lower()
		for specific in list(self.specificFonts.keys()):
			if specific.lower().find( testname ) > -1:
				if single:
					return specific
				result[ self.specificFonts[specific][FONTNAME]]=1
		for majorFamily in list(self.families.keys()):
			if majorFamily.lower().find( testname ) > -1:
				...
				for minorFamily in list(self.families[majorFamily].keys()):
