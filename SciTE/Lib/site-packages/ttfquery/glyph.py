...
def decomposeOutline( contour, steps=3 ):
	...
	if len(contour)<3:
		return ()
	set = contour[:]
	def on(xxx_todo_changeme ):
		"""Is this record on the contour?"""
		((Ax,Ay),Af) = xxx_todo_changeme
		return Af==1

	def merge(xxx_todo_changeme1, xxx_todo_changeme2):
		"""Merge two off-point records into an on-point record"""
		((Ax,Ay),Af) = xxx_todo_changeme1
		((Bx,By),Bf) = xxx_todo_changeme2
